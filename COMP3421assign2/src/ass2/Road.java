...
public class Road extends GameObject {
	...
	private void generateRoad() {
		...
		for (double t = increment; t < (size() - increment); t = t + increment) {
			...
			// scale normals
			double normalMagnitude = Math.sqrt(normal1[0] * normal1[0] + normal1[1] * normal1[1]); // magnitude
																									// should
																									// be
																									// same
																									// for
			...
			normal1[1] *= width() / 2;
			// System.out.println("normal length: "+ Math.sqrt(
			// normal1[0]*normal1[0] + normal1[1]*normal1[1] ) / width());
	...
	public void drawSelf(GL2 gl) {
		...
		/*
		 * gl.glBegin(GL2.GL_LINES);{ for(int i = 0; i<myPoints.size()-1;i+=2){
		...
		{
			int altitudeIndex = 0;
			for (int i = 0; i < roadPoints.size() - 8; i += 4) { // draw one
																	// road
																	// segment
				// System.out.println(roadPoints.get(i) + " "
				// +roadPoints.get(i+1) );
				gl.glTexCoord2d(1, 1);
				// System.out.println( "alt " +
				// (Terrain.altitude(roadPoints.get(i),roadPoints.get(i+1)) +
				// 0.1 ));
				// System.out.println( "alt1 " +
				// Terrain.altitude(roadPoints.get(i+2),roadPoints.get(i+3)) +
				// 0.1 ));
				// System.out.println( "alt2 " +
				// Terrain.altitude(roadPoints.get(i+6),roadPoints.get(i+7) +
				// 0.2 ));
				// System.out.println( "alt3 " +
				// Terrain.altitude(roadPoints.get(i+4),roadPoints.get(i+5) +
				// 0.2 ));
				gl.glVertex3d(roadPoints.get(i), Terrain.altitude(roadPoints.get(i), roadPoints.get(i + 1)) + 0.1,
						roadPoints.get(i + 1));
				gl.glTexCoord2d(1, 0);
				gl.glVertex3d(roadPoints.get(i + 2),
						Terrain.altitude(roadPoints.get(i + 2), roadPoints.get(i + 3)) + 0.1, roadPoints.get(i + 3));
				gl.glTexCoord2d(0, 0);
				gl.glVertex3d(roadPoints.get(i + 6),
						Terrain.altitude(roadPoints.get(i + 6), roadPoints.get(i + 7)) + 0.1, roadPoints.get(i + 7));
				gl.glTexCoord2d(0, 1);
				gl.glVertex3d(roadPoints.get(i + 4),
						Terrain.altitude(roadPoints.get(i + 4), roadPoints.get(i + 5)) + 0.1, roadPoints.get(i + 5));
				altitudeIndex++;
