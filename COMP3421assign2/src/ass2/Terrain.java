...
public class Terrain extends GameObject {
	...
	public static double altitude(double x, double z) {
		...
		double temj = z - j;
		if ((temi + temj) <= 1) {
			altitude = ((heightXZ * (1 - temi) + heightX1Z * temi) * ((1 - temi - temj) / (1 - temi))
					+ ((heightXZ1 * (1 - temi) + heightX1Z * temi) * ((temj) / (1 - temi))));
		} else {
			altitude = ((heightXZ1 * (1 - temi) + heightX1Z * temi) * ((1 - temj) / temi))
					+ ((heightXZ1 * (1 - temi) + heightX1Z1 * temi) * ((temi + temj - 1) / temi));
		}
	...
	/*
	 * public void addZombie(GL2 gl) { Zombie myZombie = new Zombie(this);
	 * double randomNum = ThreadLocalRandom.current().nextDouble(0,1); double
	 * randomNumX = (randomNum * (mySize.width-1)); randomNum =
	 * ThreadLocalRandom.current().nextDouble(0,1); double randomNumY =
	 * (randomNum * (mySize.height-1));
	 * myZombie.setPosition(randomNumX,altitude(randomNumX,randomNumY)+
	 * getMyAvatarY(),randomNumY); randomNum =
	 * ThreadLocalRandom.current().nextDouble(0,1);
	 * 
	 * double random = randomNum/10; double[] scale = new
	 * double[]{0.5+random,0.45+random,0.5+random}; myZombie.setScale(scale);
	 * 
	...
	public void update(double dt) {
		...
		if (!forwardPortal) {
			...
			} else if (myAvatar.getPosition()[0] > getMySize().width - 1
					&& Math.abs(myAvatar.getPosition()[2] - PortalB) < 1) {
		...
		myAvatar.setPosition(getMySize().width - Game.positionX,
				altitude(getMySize().width - Game.positionX, getMySize().height - Game.positionZ) + getMyAvatarY(),
				getMySize().height - Game.positionZ);
		if ((myAvatar.getPosition()[0] < 0 && Math.abs(myAvatar.getPosition()[2] - PortalA) < 1)
				|| (myAvatar.getPosition()[0] > getMySize().width - 1
						&& Math.abs(myAvatar.getPosition()[2] - PortalB) < 1))
			forwardPortal = !forwardPortal;
		...
		for (Zombie element : myZombies) {
			...
			rotation = new double[] { -Game.speed * Math.sin(rotation[1] / 180 * Math.PI) * 0.5, 0,
					-Game.speed * Math.cos(rotation[1] / 180 * Math.PI) * 0.5 };
			...
			double ans = (((position[0] - rotation[0]) / Math.abs(position[0] - rotation[0])) + 2) * 90
					+ ((Math.atan((rotation[2] - position[2]) / (position[0] - rotation[0])) * (180 / Math.PI)));
			ans = ((ans + 180.0) % 360.0 + 360.0) % 360.0 - 180.0;

			if (((position[0] - rotation[0]) * (position[0] - rotation[0]))
					+ ((rotation[2] - position[2]) * (rotation[2] - position[2])) <= 30) {
				if ((ans - element.getRotation()[1] <= 180 && ans - element.getRotation()[1] > 0)
						|| ans - element.getRotation()[1] < -180) {
				...
				ans = (((position[0] - rotation[0]) / Math.abs(position[0] - rotation[0])) + 2) * 90
						+ ((Math.atan((rotation[2] - position[2]) / (position[0] - rotation[0])) * (180 / Math.PI)));
				ans = ((ans + 180.0) % 360.0 + 360.0) % 360.0 - 180.0;
				if ((ans - element.getRotation()[1] <= 180 && ans - element.getRotation()[1] > 0)
						|| ans - element.getRotation()[1] < -180) {
	...
	public void setTextures(MyTexture ground, MyTexture treeTop, MyTexture treeTrunk, MyTexture road, MyTexture avFace,
			MyTexture headTex, MyTexture bodyTex, MyTexture ATex, MyTexture BTex) {
		...
		/*
		 * for(Zombie z: myZombies){ z.setTextures(zFace,ZheadTex, ZBodyTex); }
		 */
